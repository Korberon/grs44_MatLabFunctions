...
if contains(direction,"t") 
    fg.Position(4) = (screenSize(screenIndex,4)-40)/2-3-76-7 ; 
    fg.Position(2) = screenSize(screenIndex,2)+40 + (screenSize(screenIndex,4)-40)/2+1 ; 
elseif contains(direction,"b") 
    fg.Position(4) = (screenSize(screenIndex,4)-40)/2-3-76-7 ; 
    fg.Position(2) = screenSize(screenIndex,2)+40+1 ; 
end 

if contains(direction,"l") 
    fg.Position(3) = screenSize(screenIndex,3)/2-2 ; 
    fg.Position(1) = screenSize(screenIndex,1)+1 ; 
elseif contains(direction,"r") 
    fg.Position(3) = screenSize(screenIndex,3)/2-2 ; 
    fg.Position(1) = screenSize(screenIndex,1)+1+screenSize(screenIndex,3)/2 ; 
end 

%% Scale?
if contains(direction,"T") 
    fg.Position(4) = screenSize(screenIndex,4)-40-76-1 ; 
    fg.Position(2) = screenSize(screenIndex,2)+40 ; 
    if ~contains(direction,"W") 
        fg.Position(4) = fg.Position(4) - 8 ; 
    end
end 
if contains(direction,"W") 
